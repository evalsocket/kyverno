...
docker-tag-repo-kyverno:
	@echo "docker tag $(REGISTRY)/evalsocket/$(KYVERNO_IMAGE):$(IMAGE_TAG) $(REGISTRY)/evalsocket/$(KYVERNO_IMAGE):latest"
...
docker-build-cli:
	...
	@docker build -f $(PWD)/$(CLI_PATH)/Dockerfile -t $(REGISTRY)/evalsocket/$(KYVERNO_CLI_IMAGE):$(IMAGE_TAG) $(PWD)/$(CLI_PATH)

docker-tag-repo-cli:
	@echo "docker tag $(REGISTRY)/evalsocket/$(KYVERNO_CLI_IMAGE):$(IMAGE_TAG) $(REGISTRY)/evalsocket/$(KYVERNO_CLI_IMAGE):latest"
	@docker tag $(REGISTRY)/evalsocket/$(KYVERNO_CLI_IMAGE):$(IMAGE_TAG) $(REGISTRY)/evalsocket/$(KYVERNO_CLI_IMAGE):latest

docker-push-cli:
	@docker push $(REGISTRY)/evalsocket/$(KYVERNO_CLI_IMAGE):$(IMAGE_TAG)
	@docker push $(REGISTRY)/evalsocket/$(KYVERNO_CLI_IMAGE):latest
...
# guidance https://github.com/evalsocket/kyverno/wiki/Generate-a-Release
release: 
	kustomize build ./definitions > ./definitions/install.yaml
